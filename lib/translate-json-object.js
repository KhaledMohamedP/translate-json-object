...
function TranslateJSONObject() {
  ...
  function init(options) {
    ...
    if(setting.googleApiKey) {
      translate = require('./service/google.js').init(setting);
    }

    if(setting.yandexApiKey) {
      translate = require('./service/yandex.js');
    }

    translate.init(setting)
  }
  ...
  function recurisveTranslateObject(destObj, srcObj, language) {
    ...
    _.forEach(srcObj, function(value, key) {
      if (_.isPlainObject(value)) {
        ...
        promises.push(translate.string(language, key, destObj));
      } else if (_.isArray(value)) {
        promises.push(translate.array(language, key, destObj, value));
      }
    });
  }
