...
function translateObject(language, key, destObj, keysArray, valuesArray) {
	return new Promise(((resolve, reject) => {
		...
		translateService.translate(valuesStr, {to: language}, (err, res) => {
			if (err || res.code !== 200) {
				reject(err || res);
			} else {
				// Yandex return an array string and so we need split back to the normal vlues
				const result = res.text[0].split(hashSimple);
				for (const [i, element] of keysArray.entries()) {
					destObj[key][element] = result[i];
				}
...
function translateString(language, key, destObj, valueStr) {
	return new Promise(((resolve, reject) => {
		translateService.translate(valueStr, {to: language}, (err, res) => {
			if (err || res.code !== 200) {
				reject(err || res);
			} else {
				// Yandex return an array string and so we need split back to the normal vlues
